...
import com.example.androidacademy.entity.NewsItem.Category;
...
public class DataUtils {
    ...
    final static Category darwinAwards = new Category(1, "Darwin Awards");
    final static Category criminal = new Category(2, "Criminal");
    final static Category animals = new Category(3, "Animals");
    final static Category music = new Category(4, "Music");

    public static List<NewsItem> generateNews(final StringResWrapper stringResWrapper) {
        Date date;
        if (newsCache.size() == 0) {
            newsCache = new ArrayList<>();
            date = createDate(2018, 10, 15, 10, 34);
            newsCache.add(new NewsItem(
                    "Tourist filmed sitting on 5m-long crocodile",
                    "https://e3.365dm.com/18/09/736x414/skynews-crocodile-australia_4433218.jpg",
                    darwinAwards,
                    date,
                    formattedDate(stringResWrapper, date),
            ...
            date = createDate(2018, 10, 14, 12, 45);
            newsCache.add(new NewsItem(
            ...
            date = createDate(2018, 9, 20, 14, 4);
            newsCache.add(new NewsItem(
            ...
            date = createDate(2018, 9, 17, 12, 45);
            newsCache.add(new NewsItem(
                    ...
                    "A man who illegally took home hacking tools from his workplace at the National Security Agency, and then "
                            ...
                            + " the dataLis, the publicity damage has left Kaspersky Lab working to address customer fears in a global "
    ...
    private static String formattedDate(final StringResWrapper stringResWrapper, final Date publishDate) {
        final Calendar today = Calendar.getInstance();
        today.set(Calendar.HOUR_OF_DAY, 0);
        today.set(Calendar.MINUTE, 0);
        final Calendar publishCalendar = Calendar.getInstance();
        publishCalendar.setTime(publishDate);
        publishCalendar.set(Calendar.HOUR_OF_DAY, 0);
        publishCalendar.set(Calendar.MINUTE, 0);
        long diff = ((today.getTime().getTime() / 1000L) - (publishCalendar.getTime().getTime() / 1000L)) / 86400;
        final SimpleDateFormat dateFormat;
        if (diff == 0) {
            long publishTimestamp = publishDate.getTime() / 1000L;
            long currentTimestamp = System.currentTimeMillis() / 1000L;
            long hourDiff = (currentTimestamp - publishTimestamp) / (60 * 60);
            dateFormat = new SimpleDateFormat("hh:mm aaa", Locale.US);
            return String.valueOf(hourDiff) + " " +
                    stringResWrapper.getString(R.string.hour_ago) +
                    ", " +
                    dateFormat.format(publishDate);
        } else if (diff == 1) {
            dateFormat = new SimpleDateFormat("hh:mm aaa", Locale.US);
            return stringResWrapper.getString(R.string.yesterday) + ", " + dateFormat.format(publishDate);
        } else {
            dateFormat = new SimpleDateFormat("MMM dd, hh:mm aaa", Locale.US);
            return dateFormat.format(publishDate);
        }
    }

    public static NewsItem getNewsByPosition(final int newsPosition) {
        return newsCache.get(newsPosition);
    }
}
